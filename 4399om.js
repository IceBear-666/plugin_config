	...
	$('.quick-write').click(function() {
		...
		var n = {
			...
			lastCompy: function() {
				...
				return $('.infofulltimejobs').each(function(e, n) {
					var i = $('.infofulltimejobs.mt_endDate').eq(e).html().split('-');
					i = parseInt(i[0]) + parseInt(i[1]);
					var r = $('.infofulltimejobs.mt_companyName').eq(e).html();
					t.push({
						year: i,
						name: r
					})
		...
		q.textWrite(function() {
			...
			q.to.each(function(e, i) {
				if (t < e + 1) return ! 1;
				var r = q.from.eq(t - e -1).find('.mt_schoolName').html();
				$(i).find('.input_txt.input_w2').eq(2).val(r);
				var a = n.addZero(q.from.eq(t - e -1).find('.mt_startYear').html() + '-1'),
				o = n.addZero(q.from.eq(t - e -1).find('.mt_endYear').html() + '-1');
				$(i).find('.input_txt.input_w2').eq(0).val(a),
				$(i).find('.input_txt.input_w2').eq(1).val(o);
				var m = q.from.eq(t - e -1).find('.mt_professional').html(),
				l = q.from.eq(t - e -1).find('.mt_education').html();
