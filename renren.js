...
if (activeId == 0 && $('#sexMale').length) {
	...
	autoWrite.custonFunc(function() {
		...
		var engLevel = help.maxEngLevel().level;
		if (engLevel > 0 && $('#EnglishLevel').length) {
			var opt = $('#EnglishLevel').get(0).options;
			opt[engLevel].selected = true;
		}
