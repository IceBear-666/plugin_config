(function() {
	...
	function() {
		...
		if ($.trim($('.xyz-form-content-item').eq(1).find('h2').text()) == '个人基本信息') {
			...
			if (basic.find($('select[name=\"highest_degree\"]')).length && $('#mt_edu').html()) {
				...
				case '博士研究生':
					...
					break;
				case '硕士研究生':
					sigNation.options[2].selected = true;
					break;
				case '大学本科-一本':
				case '大学本科-二本':
				case '大学本科-三本':
			...
			if (basic.find($('input[name=\"height\"]')).length) {
				basic.find($('input[name=\"height\"]')).val($('#mt_height').html())
			}
			if (basic.find($('input[name=\"live_city\"]')).length) {
				basic.find($('input[name=\"live_city\"]')).val($('#mt_livecity_pro').html())
			}
		...
		if ($.trim($('.xyz-form-content-item').eq(2).find('h2').text()) == '教育经历') {
		...
		if ($.trim($('.xyz-form-content-item').eq(4).find('h2').text()) == '实习经历' && $('.infoparttimejobs').length != 0) {
			$('.item-choose').eq(0).find('span').eq(1).click();
			window.setTimeout(function() {
				for (var i = 0; i < $('.infoparttimejobs').length; i++) {
					var job = $('.xyz-form-content-item').eq(4);
		...
		if ($.trim($('.xyz-form-content-item').eq(5).find('h2').text()) == '项目经验' && $('.infoproject').length != 0) {
			$('.item-choose').eq(1).find('span').eq(1).click();
			window.setTimeout(function() {
				for (var i = 0; i < $('.infoproject').length; i++) {
					var ject = $('.xyz-form-content-item').eq(5);
		...
		if ($.trim($('.xyz-form-content-item').eq(7).find('h2').text()) == '语言能力' && $('.infoproject').length != 0) {
			for (var i = 0; i < $('.infoOtherSkill').length; i++) {
				var lang = $('.xyz-form-content-item').eq(7);
		...
		if ($.trim($('.xyz-form-content-item').eq(9).find('h2').text()) == '荣誉奖励' && $('.infoschaward').length != 0) {
			$('.item-choose').eq(2).find('span').eq(1).click();
			window.setTimeout(function() {
				for (var i = 0; i < $('.infoschaward').length; i++) {
					var award = $('.xyz-form-content-item').eq(9);
		...
		if ($.trim($('.xyz-form-content-item').eq(7).find('h2').text()) == '学生组织任职经历') {
			$('.item-choose').eq(3).find('span').eq(1).click();
			window.setTimeout(function() {
				for (var i = 0; i < $('.infoschoolclub').length; i++) {
					var club = $('.xyz-form-content-item').eq(7);
					...
					if (club.find($('input[name=\"organize\"]')).eq(i).length && $('.infoschoolclub .mt_positionName').eq(i).html()) {
						club.find($('input[name=\"organize\"]')).eq(i).val($('.infoschoolclub .mt_positionName').eq(i).html())
					}
					if (club.find($('textarea[name=\"duty\"]')).eq(i).length && $('.infoschoolclub .mt_workDes').eq(i).html()) {
						club.find($('textarea[name=\"duty\"]')).eq(i).val($('.infoschoolclub .mt_workDes').eq(i).html())
					}
				}
			},
			200)
		}
		if ($.trim($('.xyz-form-content-item').eq(10).find('h2').text()) == '活动经历') {
			$('.item-choose').eq(3).find('span').eq(1).click();
			window.setTimeout(function() {
				for (var i = 0; i < $('.infoschoolclub').length; i++) {
					var club = $('.xyz-form-content-item').eq(10);
					if (club.find($('input[name=\"name\"]')).eq(i).length && $('.infoschoolclub .mt_schClubName').eq(i).html()) {
						club.find($('input[name=\"name\"]')).eq(i).val($('.infoschoolclub .mt_schClubName').eq(i).html())
					}
					if (club.find($('input[name=\"begin_time\"]')).eq(i) && $('.infoschoolclub .mt_startDate').eq(i).html()) {
		...
		if ($.trim($('.xyz-form-content-item').eq(12).find('h2').text()) == '兴趣特长') {
			var hobby = $('.xyz-form-content-item').eq(12);
		...
		if ($.trim($('.xyz-form-content-item').eq(14).find('h2').text()) == '自我描述') {
			var desc = $('.xyz-form-content-item').eq(14);
			if (desc.find($('textarea[name=\"desc\"]')).length && $('#mt_selfIntro').html()) {
				desc.find($('textarea[name=\"desc\"]')).val($('#mt_selfIntro').html())
			}
		}
		if ($.trim($('.xyz-form-content-item').eq(15).find('h2').text()) == '自我描述') {
			var desc = $('.xyz-form-content-item').eq(15);
			if (desc.find($('textarea[name=\"desc\"]')).length && $('#mt_selfIntro').html()) {
				desc.find($('textarea[name=\"desc\"]')).val($('#mt_selfIntro').html())
			}
		}
