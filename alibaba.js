(function(){
	$('.quick-write').on('click',function(){
		...
		if ($('#jm_qiuck_cover')) {
			...
			setTimeout(\"$('#jm_qiuck_cover').fadeOut()\",1000);}
		...
		if($('select[name=\"graduateYear\"]').length && $('#mt_gradutetime').html()){
			...
			if(gray>=2016){
				gray=gray-2015;
				$('select[name=\"graduateYear\"]')[0].options[gray-1].selected=true;
		...
		for(var i=0;i<$('.infopledu').length;i++){
			if($('.educationxps').eq(i).length){
				...
				if($edudiv.find('select[name=\"startYear\"]') && $('.infopledu .mt_startYearY').eq(i).html()){
					...
					$edudiv.find('select[name=\"startYear\"]')[0].options[2019-y].selected=true;
				...
				if($edudiv.find('select[name=\"endYear\"]') && $('.infopledu .mt_endYearY').eq(i).html()){
					...
					$edudiv.find('select[name=\"endYear\"]')[0].options[2024-y].selected=true;
		...
		//实践活动
		var awardLength = $('#schawards-list .mt_awardsName').length;
		if(awardLength>0 && $('#schawards-list .mt_awardsName').html()){
			$('#hasCompetitions').find('option').eq(1).attr('selected', true);
		}
		
		var jobsLength = $('#infoparttimejobs').length;
		if(jobsLength>0 && $('#infoparttimejobs .metil').html()){
			$('#hasPracticeXps').find('option').eq(1).attr('selected', true);
		}
		
		//学校
		if($('#s2id_autogen3') && $('#edu-list').find('.mt_schoolName').html()){
			for(var i=0; i<$('#edu-list').find('.mt_schoolName').length; i++){
				$('#s2id_autogen3').val($('#edu-list').find('.mt_schoolName').eq(i).html());
			}
		}
